...
internal enum class Operator(private val representation: String) {
    ...
    companion object {
        @JvmStatic
        fun findValue(value: String): Operator? =
            values()
...
public data class Field(public val name: String) {
    // Not contains
    public infix fun <E : Any> notIn(value: E): Predicate = notContainsAll(listOf(value))

    public infix fun <E : Any> notContains(value: E): Predicate = notContainsAll(listOf(value))

    public fun <E : Any> notContains(vararg values: E): Predicate = notContainsAll(values.toList())

    public infix fun <E : Any> notContainsAll(values: Collection<E>): Predicate = Predicate(this, NOT_IN, values)

    // Contains
    @Suppress("ktlint:standard:function-naming")
    public infix fun <E : Any> `in`(value: E): Predicate = containsAll(listOf(value))

    public infix fun <E : Any> contains(value: E): Predicate = containsAll(listOf(value))

    public fun <E : Any> containsAll(vararg values: E): Predicate = containsAll(values.toList())

    public infix fun <E : Any> containsAll(values: Collection<E>): Predicate = Predicate(this, IN, values)

    // Equal
    public infix fun <T : Comparable<T>> eq(value: T): Predicate = equal(value)

    public infix fun <T : Comparable<T>> equal(value: T): Predicate = Predicate(this, EQUAL, value)

    // Not equal
    public infix fun <T : Comparable<T>> ne(value: T): Predicate = notEqual(value)

    public infix fun <T : Comparable<T>> notEqual(value: T): Predicate = Predicate(this, NOT_EQUAL, value)

    // Greater or equal
    public infix fun <T : Comparable<T>> ge(value: T): Predicate = greaterOrEqual(value)

    public infix fun <T : Comparable<T>> greaterOrEqual(value: T): Predicate = Predicate(this, GREATER_OR_EQUAL, value)

    // Greater than
    public infix fun <T : Comparable<T>> gt(value: T): Predicate = greaterThan(value)

    public infix fun <T : Comparable<T>> greaterThan(value: T): Predicate = Predicate(this, GREATER_THAN, value)

    // Less or equal
    public infix fun <T : Comparable<T>> le(value: T): Predicate = lessOrEqual(value)

    public infix fun <T : Comparable<T>> lessOrEqual(value: T): Predicate = Predicate(this, LESS_OR_EQUAL, value)

    // Less than
    public infix fun <T : Comparable<T>> lt(value: T): Predicate = lessThan(value)

    public infix fun <T : Comparable<T>> lessThan(value: T): Predicate = Predicate(this, LOWER_THAN, value)
...
public class PredicateVerifier<T : Comparable<T>>
    @PublishedApi
...
public class CollectionPredicateVerifier<T : Any>
    @PublishedApi
    internal constructor(
        field: String,
...
private fun fromString(
    ...
    when (klass) {
        ...
        Instant::class -> Instant.parse(value)
            ...
            klass.constructors.firstOrNull { it.parameters.size == 1 && it.parameters.first().type == typeOf<String>() }
                ?.call(value)
    }
